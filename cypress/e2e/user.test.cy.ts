...
describe('@POST register user', () => {
  it('OK @201', () => {
    ...
    // When
    Register(user).then((response: Cypress.Response<User>) => {
      // Then
  ...
  it('KO @422 : empty username', () => {
    ...
    // When
    Register(user).then(
      (response: Cypress.Response<{ errors: { username: string[] } }>) => {
        // Then
  ...
  it('KO @422 : empty email', () => {
    ...
    // When
    Register(user).then(
      (response: Cypress.Response<{ errors: { email: string[] } }>) => {
        // Then
        expect(response.status).to.equal(422);
  ...
  it('KO @422 : empty password', () => {
    ...
    // When
    Register(user).then(
      (response: Cypress.Response<{ errors: { password: string[] } }>) => {
        // Then
        expect(response.status).to.equal(422);
  ...
  it('KO @422 : existing credentials', () => {
    ...
    Register(user).then(
      (
        ...
        // Then
        expect(response.status).to.equal(422);
        expect(response.body.errors.username[0]).to.equal(
...
describe('@POST login', () => {
  ...
  it('KO @422 : incorrect password', () => {
    ...
    Login({
      ...
      expect(response.body.errors['email or password'][0]).to.equal(
        'is invalid'
      );
    });
  });
...
describe('@GET current user', () => {
  it('OK @200', () => {
    ...
    cy.then(function () {
      cy.getRequest('/api/user', this.token).then(
